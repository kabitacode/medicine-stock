...
import { fetchObatMasuk, fetchSupplier, fetchSupplierDelete } from '@/services';
...
const Page: React.FC = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const apiData = await fetchObatMasuk(user?.token);
    ...
    const handleEdit = (id: string) => {
        router.push(`/obat-masuk/edit/${id}`);
    };
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const apiData = await fetchSupplierDelete(user?.token, id);
    ...
    const filteredData = data.filter(item =>
        ...
        item.supplier.nama.toLowerCase().includes(searchQuery.toLowerCase()) 
    );
    ...
    return (
        <DashboardLayout>
            ...
            <div className="flex mt-4 mr-5 ml-5 mb-5 justify-between">
                <h1 className="text-2xl font-semibold">Transaksi Obat Masuk</h1>

                <div className="">
                    <Link href={'/obat-masuk/add'}>
                ...
                <Table>
                    <TableHead className='bg-green-700'>
                        <TableRow>
                            ...
                            <TableCell sx={{ color: 'white', fontWeight: '600' }}>Penerima</TableCell>
                            <TableCell sx={{ color: 'white', fontWeight: '600' }}>Tanggal Masuk</TableCell>
